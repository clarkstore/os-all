...
package cn.onestop.wx.mini.util;

import cn.binarywang.wx.miniapp.api.WxMaService;
import cn.binarywang.wx.miniapp.bean.WxMaSubscribeMessage;
import cn.onestop.wx.mini.util.dto.SubscribeConfigs;
import cn.onestop.wx.mini.util.dto.SubscribeDto;
import cn.onestop.wx.mini.util.dto.SubscribeReqDto;
...
public class OsWxMiniUtils {
     ...
     * 获取小程序码（永久有效、数量暂无限制）
     * 供自定义接口使用，必传filepath
     * @param scene <P>最大32个可见字符，只支持数字，大小写英文以及部分特殊字符：!#$&'()*+,/:;=?@-._~，其它字符请自行编码为合法字符（因不支持%，中文无法使用 urlencode 处理，请使用其他编码方式）</P>
